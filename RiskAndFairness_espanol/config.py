...
def shuffle(data):
    ...
    for x in range(0,8):
        if Player.participant.id_in_session == x:
            session = list_sessions[x]
        elif Player.id_in_session > 7:
            new_id = Player.id_in_session - 7
            if Player.id_in_session == new_id:
                session = list_sessions[new_id]
        elif Player.id_in_session > 15:
            new_id = Player.id_in_session - 15
            if Player.id_in_session == new_id:
                session = list_sessions[new_id]
        elif Player.id_in_session > 23:
            new_id = Player.id_in_session - 23
            if Player.id_in_session == new_id:
                session = list_sessions[new_id]
        elif Player.id_in_session > 31:
            new_id = Player.id_in_session - 31
            if Player.id_in_session == new_id:
                session = list_sessions[new_id]

    
    sorted_data = [i for _,i in sorted(zip(session,data))]
    print(session)
